...
# TIF -> GRD
region_all  = PyGMTPlotter.load_tif_to_grd(tif_path=tif_sbas_allregion, grd_path=grd_sbas_allregion)
region_sbas = PyGMTPlotter.load_tif_to_grd(tif_path=tif_sbas, grd_path=grd_sbas)

# TXT -> GRD
region_sigma = PyGMTPlotter.load_txt_to_grd(txt_path=txt_sigma, grd_path=grd_sigma, space=0.0005)
region_psi   = PyGMTPlotter.load_txt_to_grd(txt_path=txt_psi, grd_path=grd_psi, space=0.0005)
...
region_sbas = plotter.region_from_grid(grd_sbas)
(plotter
    .new()  
    ...
    .add_colorbar_bottom()
    .add_scale_top(region=region_sbas, projection=projection)
    .add_rose_right(region=region_sbas, projection=projection)
    .save(output_sbas)
)
...
region_sigma = plotter.region_from_grid(grd_sigma)
(plotter
    .new() # 应用新的默认样式
    .draw_basemap(region=region_sigma, projection=projection, title="Deformation: SIGMA")
    .draw_dem(dem_tif=tif_dem, region=region_sigma, projection=projection)
    .draw_deformation(
        data_grd=grd_sigma,
        cpt=cpt_saga,
...
region_psi = plotter.region_from_grid(grd_psi)
(plotter
    .new()  # 应用新的默认样式
    ...
    .add_colorbar_bottom()
    .add_scale_top(region=region_psi, projection=projection)
